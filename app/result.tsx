...
export default function ResultScreen() {
  ...
  const loadIoTImage = async () => {
    try {
      ...
      try {
        ...
        console.log(`✅ Image downloaded in ${(imageDownloadDuration / 1000).toFixed(1)}s, size: ${(imageBlob.size / 1024).toFixed(1)} KB`);
      ...
      if (result.data.photoId) {
        console.log('📥 Loading photo from MongoDB with photoId:', result.data.photoId);
        ...
        if (result.data.fieldId) {
          ...
          console.log('✅ Added fieldId from API response:', result.data.fieldId);
        }
  ...
  const generateMonitoringPlanBackground = async (): Promise<boolean> => {
    ...
    try {
      ...
      console.log('✅ Monitoring plan generated:', aiResponse.substring(0, 100));
      ...
      console.error('❌ Error generating monitoring plan in background:', error);
      return false;
    }
  };
          ...
          initialImage={(() => {
            ...
            if (imageUrl && imageUrl.includes('firebasestorage.googleapis.com/v0/b/') && 
              ...
              if (urlParts.length === 2) {
                ...
                console.log('✅ Fixed URL:', imageUrl.substring(0, 80) + '...');
              }
            }
